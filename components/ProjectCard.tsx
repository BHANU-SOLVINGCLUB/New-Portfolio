...
export function ProjectCard({ project }: { project: Project }) {
  return (
    <Card className="group hover:border-primary/50 transition-colors overflow-hidden max-h-[calc(100vh-200px)] sm:max-h-[calc(100vh-220px)] md:max-h-[calc(100vh-200px)] flex flex-col rounded-2xl border-[#3f3f3f] flex-1 min-w-0" style={{ background: "linear-gradient(0deg, #141414, #242424)" }}>
      <div className="grid md:grid-cols-2 gap-0 flex-1 min-h-0">
        {/* Left Section - Visual/Image */}
        <div
          className={`relative p-4 sm:p-6 md:p-8 lg:p-12 flex items-center justify-center min-h-[200px] sm:min-h-[250px] md:min-h-0 md:max-h-full order-1 md:order-1 ${
                ...
                <div className="bg-muted rounded-[1.5rem] aspect-[9/19] overflow-hidden flex items-center justify-center">
                  {project.image && project.image !== "" && (project.image.startsWith("data:") || project.image.startsWith("https://")) ? (
                    ...
                    <div className="text-center p-2 sm:p-3 md:p-4">
                      <div className="text-sm sm:text-base md:text-lg lg:text-xl xl:text-2xl font-bold mb-1 sm:mb-2 line-clamp-2">
                        {project.title}
                      </div>
        ...
        <div className="p-4 sm:p-5 md:p-6 lg:p-8 flex flex-col min-h-0 flex-1 order-2 md:order-2">
          ...
          <div className="flex flex-col sm:flex-row gap-2 sm:gap-2 md:gap-3 mt-auto pt-2 sm:pt-3 md:pt-4 border-t border-border flex-shrink-0">
            ...
            {project.github ? (
              <Button variant="outline" asChild className="w-full sm:flex-1 md:w-auto text-xs sm:text-sm py-2 sm:py-2">
                <a href={project.github} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-1.5 sm:gap-2">
                  <Github className="h-3 w-3 sm:h-4 sm:w-4" />
                  <span>GitHub</span>
                </a>
              </Button>
            ) : (
              <Button variant="outline" disabled className="w-full sm:flex-1 md:w-auto text-xs sm:text-sm opacity-50 cursor-not-allowed py-2 sm:py-2">
            ...
            {project.category === "mobile" && (project.appStore || project.playStore) ? (
              <Dialog>
                <DialogTrigger asChild>
                  <Button variant="outline" className="w-full sm:flex-1 md:w-auto text-xs sm:text-sm py-2 sm:py-2">
              ...
              <Button variant="outline" asChild className="w-full sm:flex-1 md:w-auto text-xs sm:text-sm py-2 sm:py-2">
                <a href={project.live} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-1.5 sm:gap-2">
